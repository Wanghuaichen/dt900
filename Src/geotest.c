...
void geotest()
{
	...
	step1();
	sprintf(str,"Resistence:%d",geophone.resi);
...
void step1()
{
	...
	for(i=0;i<64;i++)
		val+=AD7190Read();
	val /= 64;
	geophone.resi = (uint32_t)round(R_ref*4*abs(val-ADCMID)/(volt/5*0xffffff));
...
void step2()
{
	...
	geophone.sens = sqrt(2*PI*geophone.freq*geo.M/1000*abs(a1-ADCMID)*4/0xffffff*5.0/(volt/R_ref)*exp(atan(t)/t));
...
void step3()
{
  ...
  HAL_Delay(T_DELAY);
	...
	geophone.impe = (uint32_t)round((R_ref*4*Inbuff[1]/(N/2)/(mag/5*0xffffff)));
