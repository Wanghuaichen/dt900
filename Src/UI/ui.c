...
void UIEventManager()
{
	if(UIInfo.keyCombo>30000 || (UIInfo.keyCombo==300 && UIInfo.KeyEvent==KEY_CANCEL))
	{
		DP_EN(0);LCD_PWR(0);
	}
...
void drawButton(struct UIWidget* widget)
{
	...
	GUI_SetTextAlign(GUI_TA_HCENTER | GUI_TA_VCENTER);
	GUI_DispStringAt(widget->widgetTitle,240,(widget->rect.y1+widget->rect.y0)/2);
}
